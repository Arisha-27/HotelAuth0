...
import { fetchAgents, fetchAgentStatus, fetchAgentTrace, fetchAgentMemory, fetchBrainInfo } from '../services/api'
import './Agents.css'
...
function AgentDetailCard({ agent, onClose }) {
  ...
  // Fetch real trace & memory from backend
  useEffect(() => {
    if (!agent?.id) return
    fetchAgentTrace(agent.id, 5)
      .then(data => {
        if (data?.trace?.length) setLiveTrace(data.trace)
      })
      .catch(() => {})
    fetchAgentMemory(agent.id)
      .then(data => setLiveMemory(data))
      .catch(() => {})
  }, [agent?.id])

  const tasks = liveTrace.length > 0
    ? liveTrace.map((t, i) => ({
        id: `T-${i}`,
        name: t.input || t.action || 'Agent task',
        status: t.status || 'completed',
        time: t.timestamp ? new Date(t.timestamp).toLocaleTimeString() : `${i + 1}m ago`,
      }))
    : [
  ...
  return (
    <div className="agent-detail-card card">
      ...
      <div className="card-body">
        ...
        {/* Memory context from backend */}
        {liveMemory?.context && (
          <div className="agent-info-row" style={{ marginTop: 8 }}>
            <div className="agent-info-item" style={{ flex: '1' }}>
              <span className="info-label">MEMORY CONTEXT</span>
              <span className="info-value font-mono text-xs" style={{ opacity: 0.7 }}>
                {typeof liveMemory.context === 'string' ? liveMemory.context.slice(0, 100) : JSON.stringify(liveMemory.context).slice(0, 100)}
              </span>
            </div>
          </div>
        )}
        ...
        <div className="agent-tasks-section">
          <button className="agent-tasks-toggle" onClick={() => setShowTasks(!showTasks)}>
            <span className="uppercase text-xs" style={{ letterSpacing: '0.1em' }}>Recent Tasks{liveTrace.length > 0 ? ' (LIVE)' : ''}</span>
    ...
    <div className="agents-page">
      <div className="agents-header">
        ...
        <div className="agents-summary font-mono text-xs text-muted">
          ...
          {brainInfo?.provider && ` • BRAIN: ${brainInfo.provider.toUpperCase()}`}
        </div>
      </div>
